...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { spaceId }: RestoreSpaceRequest = await req.json()
    ...
    if (updateError) {
      console.error('❌ Error restoring space:', updateError)
      return new Response(
        JSON.stringify({ error: 'Failed to restore space', details: updateError.message }),
        { 
          status: 500, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    console.error('❌ Unexpected error in restore-space function:', error)
    return new Response(
