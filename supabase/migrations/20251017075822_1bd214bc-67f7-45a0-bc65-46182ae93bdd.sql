-- FASE 1.2: Add price validation to validate_and_reserve_slot
-- This prevents price tampering by validating client-side calculations server-side

DROP FUNCTION IF EXISTS public.validate_and_reserve_slot(
  uuid, date, time, time, uuid, integer, text
);
...
BEGIN
  ...
  v_duration_hours := EXTRACT(EPOCH FROM (start_time_param - end_time_param)) / 3600.0;
  v_duration_hours := ABS(v_duration_hours); -- Ensure positive
  ...
  -- 🔒 CRITICAL: Validate client price against server calculation
