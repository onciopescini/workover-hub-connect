-- Fix price calculation to include guests_count in validate_and_reserve_slot
-- Issue: Server-side price calculation was not multiplying by guests_count_param
-- This caused "Price mismatch" errors when booking for multiple guests
...
BEGIN
  ...
  -- FIXED: Multiply by guests_count_param for accurate per-person pricing
  IF v_duration_hours >= 6 THEN
    v_server_price := v_space.price_per_day * guests_count_param;
  ELSE
    v_server_price := v_space.price_per_hour * v_duration_hours * guests_count_param;
  END IF;
  ...
  -- Cast status to booking_status enum type
