-- Add suspended_at column to profiles table for host grace period
ALTER TABLE public.profiles ADD COLUMN suspended_at TIMESTAMPTZ DEFAULT NULL;

-- Create index for performance on suspended users
CREATE INDEX idx_profiles_suspended_at ON public.profiles(suspended_at) WHERE suspended_at IS NOT NULL;
