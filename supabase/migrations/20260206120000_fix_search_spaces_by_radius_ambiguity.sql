-- CRITICAL FIX: Remove overloaded search_spaces_by_radius variants and keep a single RPC signature.

-- Drop legacy overloaded variants explicitly to avoid PostgREST ambiguity.
...
-- Defensive cleanup for simplified/older variants.
DROP FUNCTION IF EXISTS public.search_spaces_by_radius(double precision, double precision, integer);
DROP FUNCTION IF EXISTS public.search_spaces_by_radius(double precision, double precision, double precision);

-- Recreate the single definitive RPC with p_radius_km as DOUBLE PRECISION.
