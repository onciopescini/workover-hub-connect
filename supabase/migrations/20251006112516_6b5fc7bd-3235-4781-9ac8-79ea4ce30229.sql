-- Create function to refresh connection suggestions for a specific user
  ...
  WITH new_suggestions AS (
    ...
    SELECT DISTINCT
      ...
      jsonb_build_object(
        ...
        'space_title', s.title,
        'shared_bookings', COUNT(DISTINCT b2.id)
      ) as shared_context
    FROM bookings b1
    ...
    GROUP BY b1.space_id, s.title, b2.user_id, p_user_id
...
GRANT EXECUTE ON FUNCTION refresh_user_suggestions(UUID) TO authenticated;