-- Drop the corrupted function
DROP FUNCTION IF EXISTS public.get_single_space_metrics(uuid);

-- Recreate the complete function
...
BEGIN
  ...
  SELECT json_agg(
    json_build_object(
      'profession', COALESCE(profession, 'Non specificato'),
