...
const SpaceDetail = () => {
  ...
  if (error) {
    sreLogger.error('Error state', { spaceId: id, component: 'SpaceDetail' }, error);
    return (
      <div className="container mx-auto py-8">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-red-600 mb-2">Errore nel caricamento</h1>
          <p className="text-gray-600 mb-4">{error.message || 'Errore sconosciuto nel caricamento dello spazio'}</p>
          {id && (
    ...
    return (
      <div className="container mx-auto py-8">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">Lo spazio che stai cercando non esiste o non è più disponibile.</p>
          {id && (
  ...
  sreLogger.debug('Rendering space details', {
    spaceId: space.id,
    hasPreLocation: Boolean(space.hasPreciseLocation),
    ...
    <div className={`container mx-auto py-8 ${isStitch ? 'bg-stitch-bg' : ''}`}>
      {isStitch ? (
        <Suspense fallback={<div className="min-h-[400px] bg-stitch-bg" />}>
          <SpaceHeroStitch>
            <SpaceDetailContent space={space} reviews={safeReviews} weightedRating={cachedRating} />
          </SpaceHeroStitch>
        </Suspense>
      ) : (
        <SpaceDetailContent space={space} reviews={safeReviews} weightedRating={cachedRating} />
